...
void setup()
{
  ...
  espData("AT+RST", 1000);                      //Resetting the ESP8266 module
  espData("AT+CWMODE=1", 1000);                 //Set the ESP mode as station mode
  espData("AT+CWJAP=\""+ mySSID +"\",\""+ myPWD +"\"", 1000);   //Connect to WiFi your network
  ...
  void loop()
  {
   ...
   int open=0;
    ...
    Serial.println(sensorData);
    Serial.println("Clap Detected");  // Clap detected
    delay(3000);
